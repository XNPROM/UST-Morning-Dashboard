...
def should_load_interpretation(summary_daily = None, blocking_quality_issues = None):
...
def main():
    ...
    print(f'  daily: {daily_panel.shape[0]} rows x {daily_panel.shape[1]} cols')
    print('Cleaning intraday data (ffill gaps, remove spikes)...')
    ...
    morning_notes = build_morning_notes(summary_daily)
    ...
    if should_load_interpretation(summary_daily, blocking_issues):
    ...
    figs = make_figures(
        ...
        daily_panel=daily_panel,
        rolling24_panel=rolling24_panel,
        asof_dt=now,
    )
