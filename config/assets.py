...
# File: assets.cpython-311.pyc (Python 3.11)
...
ASSETS: 'list[AssetConfig]' = [
    ...
    AssetConfig('cnyfix', 'E. RMB', 'RMB', 'USD/CNY fixing', ['CNY=PBOC', 'CNY=SAEC'], 'fixing_fx', 'daily', 430),
    ...
    AssetConfig('gold', 'F. Commodities', 'Commodities', 'Gold', ['XAU=', 'GCc1'], 'commodity', 'both', 530),
...
def wanted_assets(freq = None):
    if freq == 'intraday':
        return [a for a in ASSETS if a.freq in ('intraday', 'both')]
    if freq == 'daily':
        return [a for a in ASSETS if a.freq in ('daily', 'both')]
    return list(ASSETS)
