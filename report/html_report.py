...
from analytics.summary import latest_fixing_info, rows_by_section
...
SECTION_LABELS = {
    ...
    'C. Treasury Futures': '2. 美债期货、油铜金',
    'D. USD & FX': '3. 美元、外汇、人民币',
    'E. RMB': '3. 美元、外汇、人民币',
    'F. Commodities': '2. 美债期货、油铜金',
}
...
def generate_html_report(
    ...
    # Section tables from summary_daily (close-to-close)
    section_tables = {}
    ...
    for sec_label, sec_data in section_tables.items():
        ...
        cols = ['Group', 'Asset', 'Level', 'Change Text', '% Change Text', 'Obs']
        main_sections_html += f'<div class="section"><h2>{sec_data["title"]}</h2>{html_table(combined, cols)}</div>'

    # Charts
    from charts.plotly_charts import ChartSpec, SECTION_META

    def _fig_to_html(fig):
        return fig.to_html(full_html=False, include_plotlyjs=False, config={'responsive': True, 'displayModeBar': False})

    chart_sections = OrderedDict()
    for spec in figs:
        sec = spec.section
        if sec not in chart_sections:
            chart_sections[sec] = []
        chart_sections[sec].append(spec)

    def _render_chart_grid(specs):
        items = ''
        for spec in sorted(specs, key=lambda s: s.priority):
            width_class = 'full' if spec.width == 'full' else 'half'
            fig_html = _fig_to_html(spec.fig)
            items += f'<div class="chart-item {width_class}">{fig_html}</div>'
        return f'<div class="chart-grid">{items}</div>'

    charts_sections_html = ''
    for sec_id, sec_title in SECTION_META.items():
        specs = chart_sections.get(sec_id, [])
        if not specs:
            continue
        inner = _render_chart_grid(specs)
        if sec_id == 'longterm':
            charts_sections_html += f'''
    <details id="{sec_id}">
        <summary class="chart-section-title">{sec_title}</summary>
        {inner}
    </details>'''
        else:
            charts_sections_html += f'''
    <div class="chart-section" id="{sec_id}">
        <h3 class="chart-section-title">{sec_title}</h3>
        {inner}
    </div>'''
...
.chart-section-title {{ font-size: 15px; font-weight: 600; color: #0f172a; margin: 24px 0 12px 0; padding-bottom: 8px; border-bottom: 1px solid #e2e8f0; display: flex; align-items: center; cursor: default; }}
.chart-section-title::before {{ content: ''; display: inline-block; width: 3px; height: 16px; background: #2563eb; border-radius: 2px; margin-right: 8px; flex-shrink: 0; }}
.chart-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }}
.chart-item.full {{ grid-column: 1 / -1; }}
.chart-item.half {{ grid-column: span 1; }}
.chart-item .plotly-graph-div {{ width: 100%; }}
...
@media print {{ .nav-bar, .scroll-top {{ display: none !important; }} .container {{ padding: 0; }} .section, .card {{ box-shadow: none; border: 1px solid #ddd; break-inside: avoid; }} }}
@media (max-width: 1000px) {{ .cards {{ grid-template-columns: repeat(2, minmax(0, 1fr)); }} .chart-grid {{ grid-template-columns: 1fr; }} .chart-item.half {{ grid-column: 1 / -1; }} .container {{ padding: 12px 14px 48px; }} .header {{ padding: 22px 0; }} .header h1 {{ font-size: 22px; }} .nav-bar {{ padding: 0 12px; }} }}
...
<div class="container" id="top">
    <div class="header">
        ...
        <div class="subtitle">
            深圳早会版 v8 | 收盘变动口径 | 前日收盘 → 当日收盘 | 生成时间：{now_str}
        </div>
    </div>
...
document.querySelectorAll('.clean-table td').forEach(function(td) {{
    ...
    if summary_daily is not None and not summary_daily.empty:
        summary_daily.to_csv(csv_path, index=False)
