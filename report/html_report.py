...
def _ai_section(interpretation: dict[str, str] | None) -> str:
    if not interpretation:
        ...
        <div class="section">
            ...
            <div class="ai-pending">
                <p>本页尚未载入分析文本。完成对应日期的 <code>ai_interpretation</code> 文件后，再次运行即可更新这一部分。</p>
            </div>
        </div>
        """
    ...
    <div class="section">
        ...
        <p class="muted" style="margin-top:12px">以下文字依据当日数据生成，用于辅助阅读图表与摘要。</p>
    </div>
    """
...
def generate_html_report(
    ...
    grade_badges = " ".join([
        f"{_section_label(sec)}: {_grade_badge(g)}" for sec, g in sorted(grades.items())
    ])

    # Cards
    cards_html = "".join([
        metric_card("观察时点", f"{windows.asof_dt.strftime('%Y-%m-%d %H:%M')}<br><span class='muted'>Asia/Shanghai</span>"),
        metric_card("主观察区间", f"{windows.main_start.strftime('%m-%d %H:%M')} → {windows.main_end.strftime('%m-%d %H:%M')}<br><span class='muted'>上一中国收盘后至早会</span>"),
        metric_card("十年期美债", chg(summary_main, "UST 10Y")),
        metric_card("美元指数", chg(summary_main, "DXY")),
        metric_card("人民币中间价", fixing_card_html),
    ])

    notes_html = "<ul class='notes-list'>" + "".join([f"<li>{n}</li>" for n in morning_notes]) + "</ul>"
...
<div class="container">
    <div class="header">
        <h1>美债与美元市场晨间报告</h1>
        <div class="subtitle">
            深圳早会版本第六版｜主观察区间：{main_window_desc}｜
            纽约时段：{ny_window_desc}｜
            页面生成时间：{datetime.now(settings.REPORT_TZ).strftime('%Y-%m-%d %H:%M:%S')}
        </div>
    </div>
    ...
    <div class="section">
        <div class="grade-bar">数据完整性与一致性：{grade_badges}</div>
        <h2>1. 利率与通胀补偿</h2>
        {rates_tbl}
    </div>
    ...
    <div class="section">
        <h2>6. 美国重要经济数据日程</h2>
        {events_html}
        <p class="muted">维护方式：FOMC、CPI、PCE、NFP、GDP、Retail Sales 等高重要性日期可填写在 <code>config/settings.py</code> 的 <code>MANUAL_US_EVENTS</code>；周度初请仍按周四 08:30 ET 自动生成。</p>
    </div>
